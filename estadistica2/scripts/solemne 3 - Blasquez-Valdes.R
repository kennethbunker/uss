...
#data <- read_xlsx("Dropbox/GitHub/uss/estadistica2/data/blasquez_valdes.xlsx")
...
data2 <- getURL(paste0(github,"blasquez_valdes.csv")) 
...
vd  <- data$coord1D_normal_all# polarización (Y): 0=moderado <- -> 10=extremo
vi1 <- as.numeric(data$edad)  # edad - H1: a medida que aumenta la edad del constituyente (X1), la polarización aumenta (Y)
vi2 <- data$p_votos           # porcentaje de voto - H2: a medida que aumenta el porcentaje de votos del candidato (X2), la polarización disminuye (Y)
